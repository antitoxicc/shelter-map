...
function getNormalizedVerificationStatus(value) {
  return String(value || "").trim().toLowerCase() === "verified" ? "verified" : "needs_review";
}

function getVerificationLabel(value) {
  return getNormalizedVerificationStatus(value) === "verified" ? "ÐŸÐ¾Ð´Ñ‚Ð²ÐµÑ€Ð¶Ð´ÐµÐ½Ð¾" : "ÐÑƒÐ¶Ð½Ð¾ Ð¿Ñ€Ð¾Ð²ÐµÑ€Ð¸Ñ‚ÑŒ";
}
...
function createMarkerIcon(status, isSelected) {
  const isVerified = getNormalizedVerificationStatus(status) === "verified";
