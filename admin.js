...
function renderLocationVerificationOptions(selectedValue) {
  ...
  return `
    <option value="needs_review"${normalizedValue === "needs_review" ? " selected" : ""}>ÐÑƒÐ¶Ð½Ð¾ Ð¿Ñ€Ð¾Ð²ÐµÑ€Ð¸Ñ‚ÑŒ</option>
    <option value="verified"${normalizedValue === "verified" ? " selected" : ""}>ÐŸÐ¾Ð´Ñ‚Ð²ÐµÑ€Ð¶Ð´ÐµÐ½Ð¾</option>
  `;
}
...
function renderSelectedShelterPanel() {
  ...
  selectedShelterPanel.innerHTML = `
    <article class="selected-shelter-card">
      ...
      <div class="meta-line">ÐÐ´Ñ€ÐµÑ: ${escapeHtml(row.address || "Ð½Ðµ ÑƒÐºÐ°Ð·Ð°Ð½")}</div>
...
async function saveShelterEdits(form) {
  ...
  if (file) {
    try {
      setAuthMessage("Загружаем вложение и сохраняем изменения...");
  ...
  setAuthMessage("Ð¡Ð¾Ñ…Ñ€Ð°Ð½ÑÐµÐ¼ Ð¸Ð·Ð¼ÐµÐ½ÐµÐ½Ð¸Ñ...");
  const { error } = await supabase.from("shelters").update(payload).eq("id", shelterId);
